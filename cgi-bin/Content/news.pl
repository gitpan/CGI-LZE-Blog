        ...
        if(not defined $sbm or ($sbm ne translate('preview'))) {
                if(defined param('message') && defined param('headline') && defined param('thread') && defined param('catlist')) {
                        ...
                        if(defined $headline && defined $message && defined $thread && $right >= 2) {
                                my %message = (title => $headline, body => $message, thread => $thread, user => $user, cat => $cat, attach => $sra, format => $format,);
...
sub saveedit {
        if(not defined param('submit') or (param('submit') ne translate('preview'))) {
                ...
                my $format  = param('format') eq 'on' ? 'html' : 'bbcode';
                my $cat     = param('catlist');
                my %message = (thread => $thread, title => $headline, body => $body, thread => $thread, cat => $cat, attach => $sra, format => $format, id => $id, user => $user, cat => $cat);
...
sub addReply {
        ...
        if(not defined param('submit') or (param('submit') ne translate("preview"))) {
                if(param('file')) {
                        ...
                        my %reply = (title => $headline, body => $body, id => $reply, user => $user, format => $format,);
                        $database->reply(\%reply);
                }
                &saveUpload();
        } else {
...
sub showMessage {
        ...
        if($ref->{id}== $id) {
                ...
                my $window = new HTML::Template::LZE::Window(\%parameter);
...
sub preview {
        ...
        my $win        = new HTML::Template::LZE::Window(\%wparameter);
        $win->set_closeable(1);
        $win->set_collapse(1);
        $win->set_moveable(1);
