...
sub Body {
        ...
        init($settingfile);
        *settings = \$CGI::LZE::settings;
        $database = new DBI::LZE::Database;
        ...
        if($action eq 'rss') {
                ...
                my %set =
                  (path => "$settings->{cgi}{bin}/templates", style => $style, title => $title, server => $settings->{cgi}{'serverName'}, login => $logIn, size => $size, right => $right, htmlright => $settings->{htmlright}, template => 'blog.html');
                initMain(\%set);
                print Header();
                ...
                my %parameter = (path => "$settings->{cgi}{bin}/templates/", style => $style, action => $action, file => $file, right => $right, mod_rewrite => $settings->{cgi}{mod_rewrite}, template => 'lzetabwidget.html');
                my $database  = new DBI::LZE::Database();
                ...
                while(my @a = $sth->fetchrow_array()) {
                        ...
                        push @{$parameter{anchors}}, {class => $nm, style => $style, text => $a[0], href => $fm, src => $a[2], title => $a[0]};
                }
                unless ($hasCurrentlink) {
                        if(param('include')) {

                                #todo session() parameters
                                #push @{$parameter{anchors}}, {text => 'Action wurde erzeugt', class => 'currentLink', style => $style, href => "tabwidget.pl", title => 'Action wurde erzeugt'};
                        } else {
                                $sth = $dbh->prepare("select title,action from actions where `action` =  ?");
                                $sth->execute($action) or warn $dbh->errstr;
                                my @a = $sth->fetchrow_array();
                                push @{$parameter{anchors}}, {text => $a[0], class => 'currentLink', style => $style, href => 'javascript:void'};
                        }
                }
...
L<CGI> L<CGI::LZE>
L<DBI> L<DBI::LZE> L<DBI::LZE::Database>
L<HTML::Template::LZE::Main> L<HTML::Template::LZE::TabWidget>
L<HTML::Template::LZE::Window> L<HTML::Template::LZE::Pages>
L<HTML::Menu::TreeView> L<HTML::LZE::BBCODE> L<HTML::LZE::Editor>
